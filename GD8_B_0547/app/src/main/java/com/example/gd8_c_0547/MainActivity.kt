package com.example.gd8_c_0547
...
class MainActivity : AppCompatActivity() {
        ...
        try{
            ...
            try{
                ...
                for(i in 0 until jsonArrayResult.length()){
                    ...
                    modelMain.lateLoc = jsonObjectLoc.getDouble("lat")
            ...
            Toast.makeText(
                this@MainActivity,
                "Oops, ada yang tidak beres. Coba lagi dalam beberapa saat.",
                Toast.LENGTH_SHORT
            ).show()
        }
    ...
    private fun initMarker(modelList: List<ModelMain>){
        for(i in modelList.indices){
            ...
            overlayItem.add(
                OverlayItem(
                    modelList[i].strName, modelList[i].strVicinity, GeoPoint(
                        modelList[i].lateLoc, modelList[i].longLoc
                    )
                )
            )
            val info = ModelMain()
            ...
            marker.position = GeoPoint(modelList[i].lateLoc, modelList[i].longLoc)
